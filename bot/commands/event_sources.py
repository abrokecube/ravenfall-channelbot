...
from twitchAPI.type import ChatEvent, AuthScope, MissingScopeException, InvalidTokenException
...
class TwitchAPIEventSource(BaseEventSource):
    ...
    async def get_twitch_auth_instance(self, user_id: int | str, user_name: str | None = None, scopes: list[AuthScope] = TWITCH_CHANNEL_SCOPES) -> Twitch:
        ...
        while True:
            twitch = await Twitch(self.twitch_app_id, self.twitch_app_secret, target_app_auth_scope=TWITCH_APP_SCOPES)
            ...
            try:
                ...
                if save_new_tokens:
                    ...
                    if isinstance(user, TwitchUser):
                        await self.db_service.update_tokens(user.id, access_token, refresh_token, user.login)
    ...
    async def setup(self, event_manager: EventManager):
        ...
        self.bot_twitch = await self.get_twitch_auth_instance(self.bot_user_id, scopes=TWITCH_BOT_USER_SCOPES)

        LOGGER.info("Initializing twitch chat instance")
        self.chat = await Chat(self.bot_twitch, initial_channel=[x['channel_name'] for x in self.channels])
        ...
        for channel in self.channels:
            if channel.get("channel_points_redeems", False):
                channel_twitch = await self.get_twitch_auth_instance(channel['channel_id'], channel['channel_name'], TWITCH_CHANNEL_SCOPES)
