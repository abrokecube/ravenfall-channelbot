...
class GameCog(Cog):
    ...
    async def exps(self, ctx: CommandEvent, count: int = 99, *, channel: RFChannel = 'this'):
        ...
        await send_multichat_command(f"?exps {count_}", "0", "", channel.channel_id, channel.channel_name)
    ...
    async def healbump(self, ctx: CommandEvent, user: str, *, channel: RFChannel = 'this'):
        ...
        Args:
            ...
            channel: Target channel.
        """
        query_result = await channel.get_query(f"select id, island, training, sailing from players where name = '{user}'")
        if not query_result:
        ...
        while True:
            ...
            query2_result = await channel.get_query(f"select id, island, training, sailing from players where name = '{user}'")
