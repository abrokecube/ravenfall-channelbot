...
class GameCog(Cog):
    ...
    def __init__(
        self, event_manager: EventManager, rf_webops_url: str = "http://pc2-mobile:7102"
    ):
    ...
    async def update(
        ...
        for channel_ in channels:
            ...
            msg_text = (
                f"{channel_.ravenbot_prefixes[0]}town {town_boost[0].skill.lower()}"
            )
    ...
    async def rfrestartstatus(self, ctx: CommandEvent, *, channel: RFChannel = "this"):
        ...
        if restart_task.finished():
            await ctx.message.reply(
                f"Last restart task finished. (status: {restart_task.get_status().value}, ch: {ch}, gl: {gl})"
            )
            return
        time_left = restart_task.get_time_left()
        if time_left <= 0:
            await ctx.message.reply(
                f"A restart is currently in progress. (status: {restart_task.get_status().value}, ch: {ch}, gl: {gl})"
            )
            return
    ...
    async def rfrestarttoggle(self, ctx: CommandEvent, *, channel: RFChannel = "this"):
        ...
        await ctx.message.reply(
            f"Auto restart is now {'enabled' if channel.auto_restart else 'disabled'}."
        )
        if old_value == True:
            restart_task = channel.restart_task
    ...
    @command(
        ...
        help_text="Postpones a restart task",
        aliases=[
            "rfrestart postpone",
            "rf restart postpone",
        ],
    ...
    async def rfrestartpostpone(
        self, ctx: CommandEvent, seconds: int = 5 * 60, *, channel: RFChannel = "this"
    ):
    ...
    async def rfrestart(
        ...
        *,
        ...
        Args:
            channel: Target channel.
        """
        channel.queue_restart(
            seconds, label=reason, reason=RestartReason.USER, overwrite_same_reason=True
        )
    ...
    @command(
        ...
        help_text="Toggles the bot monitoring",
        aliases=[
            "toggle bot monitor",
            "botmonitor toggle",
        ],
    ...
    async def togglebotmonitor(
        ...
        if all_:
            for channel_ in self.global_context.require_service(
                RFChannelManager
            ).channels:
                channel_.monitoring_paused = channel.monitoring_paused
            await ctx.message.reply(
                "RavenBot monitoring is now "
                + ("PAUSED" if channel.monitoring_paused else "RESUMED")
                + " for all channels."
            )
        else:
            await ctx.message.reply(
    ...
    async def ds(self, ctx: CommandEvent, *, channel: RFChannel = "this"):
        ...
        if queue_len > 0:
            raise CommandError(
                f"There {pl2(queue_len, 'is', 'are', False)} currently {pl(queue_len, 'scrolls')} in the queue. Wait until the queue is empty."
            )
        ...
        await send_multichat_command(
            "?ds", "0", "", channel.channel_id, channel.channel_name
        )
    ...
    async def rs(self, ctx: CommandEvent, *, channel: RFChannel = "this"):
        ...
        if channel.event == RFChannelEvent.RAID:
            raise CommandError("There is currently an active raid")
        await send_multichat_command(
            "?rs", "0", "", channel.channel_id, channel.channel_name
        )

    @command(name="exps")
    @parameter("channel", aliases=["channel", "c"], converter=RFChannelConverter)
    async def exps(
        self, ctx: CommandEvent, count: int = 99, *, channel: RFChannel = "this"
    ):
        ...
        if m_server.multiplier > 1:
            ...
            s_remaining = (
                m_server.end_time - datetime.now(tz=timezone.utc)
            ).total_seconds()
        else:
            s_duration = 0
            s_remaining = 0
        ...
        if count_ > count:
            raise CommandError(
                f"The number of scrolls used must result in a 100x multiplier. ({count_} scrolls are needed)"
            )
        if count_ > scrolls_remaining:
            raise CommandError(
                f"There are not enough scrolls in stock to reach 100x multiplier. (stock: {scrolls_remaining}, required: {count_})"
            )

        elif multiplier_value > 1 and remaining / duration < 0.8:
            raise CommandError(
                "Wait for the current multiplier to expire before using this command again."
            )

        await send_multichat_command(
            f"?exps {count_}", "0", "", channel.channel_id, channel.channel_name
        )
    ...
    async def fs(self, ctx: CommandEvent, *, channel: RFChannel = "this"):
        ...
        if f["boost"]["isactive"]:
            raise CommandError(
                f"There is currently an active ferry boost, ending in {format_seconds(f['boost']['remainingtime'], size=TimeSize.LONG, include_zero=False)}."
            )
        await send_multichat_command(
            "?fs", "0", "", channel.channel_id, channel.channel_name
        )
    ...
    async def scrolls_aliases(self, ctx: MessageEvent, result: re.Match):
        await self.event_manager.execute_text(
            f"channelscrolls{result.group('args')}", ctx
        )
    ...
    async def restockscrolls(
        self, ctx: CommandEvent, item: Item, count: int, *, channel: RFChannel = "this"
    ):
        ...
        if item.id not in (
            Items.ExpMultiplierScroll.value,
            Items.RaidScroll.value,
            Items.DungeonScroll.value,
        ):
        ...
        try:
            async with self.web_op_lock:
                result = await self.rf_webops.redeem_items(
                    item_id_map[item.id], count, char_list
                )
        except asyncio.TimeoutError:
            raise CommandError("Task timed out.")
        if result["status"] == "success":
            await ctx.message.reply(
                f"Successfully restocked {sum(result['redeemed'].values())}x {item.name}."
            )
        else:
    ...
    @checks(MinPermissionLevel(UserRole.MODERATOR))
    async def healbump(
        self, ctx: CommandEvent, user: str, *, channel: RFChannel = "this"
    ):
        ...
        Args:
            ...
            channel: Target channel.
        """
        query_result = await channel.get_query(
            f"select id, island, training, sailing from players where name = '{user}'"
        )
        if not query_result:
            raise CommandError("User not found")
        if query_result["sailing"]:
            raise CommandError("User is sailing")
        sender = await channel.build_sender_from_character_id(query_result["id"])
        await send_to_server(
            channel.middleman_connection_id, RavenBotTemplates.sail(sender)
        )
        await send_to_server(
        ...
        while True:
            ...
            query2_result = await channel.get_query(
                f"select id, island, training, sailing from players where name = '{user}'"
            )
            if not query2_result:
                raise CommandError("User not found")
            if not query2_result["sailing"]:
                break
