...
async def get_prometheus_series(url: str, query: str, duration_s: int, step_s: int = 20):
    now = time.time()
...
async def get_prometheus_instant(url: str, query: str) -> List[PromethusInstantResult] | None:
    async with aiohttp.ClientSession() as session:
        r = await session.get(
