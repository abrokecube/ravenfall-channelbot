...
class RavenfallMultichatClient:
    ...
    async def _get[T](self, url_suffix: str, out_type: type[T], timeout_seconds: int = 3) -> T:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout_seconds)) as session:
            try:
                ...
                logger.exception(f"Error fetching from {self.base_url}: {e}")
                raise

    async def _post(self, url_suffix: str, payload: dict[str, Any], timeout_seconds: int = 3) -> None:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout_seconds)) as session:
            try:
                ...
                logger.exception(f"Error posting to {self.base_url}: {e}")
                raise
    ...
    async def get_desync_info(self, *, timeout_seconds: int = 3) -> DesyncResponse:
        ...
        """
        return await self._get("get_desync", DesyncResponse, timeout_seconds=timeout_seconds)


    async def get_total_item_count(self, *, timeout_seconds: int = 3) -> TotalItemCountResponse:
        ...
        """
        return await self._get("get_total_item_count", TotalItemCountResponse, timeout_seconds=timeout_seconds)
    ...
    async def get_char_info(self, *, timeout_seconds: int = 3) -> CharInfoResponse:
        ...
        """
        return await self._get("get_char_data", CharInfoResponse, timeout_seconds=timeout_seconds)


    async def get_char_items(self, channel_id: str, *, timeout_seconds: int = 3) -> CharItemsResponse:
        ...
        """
        return await self._get(f"get_char_items/{channel_id}", CharItemsResponse, timeout_seconds=timeout_seconds)


    async def get_char_coins(self, channel_id: str, *, timeout_seconds: int = 3) -> CharCoinsResponse:
        ...
        """
        return await self._get(f"get_char_coins/{channel_id}", CharCoinsResponse, timeout_seconds=timeout_seconds)


    async def get_scroll_counts(self, channel_id: str, *, timeout_seconds: int = 3) -> ScrollsResponse:
        ...
        """
        return await self._get(f"get_scrolls/{channel_id}", ScrollsResponse, timeout_seconds=timeout_seconds)
