...
class TimeSize(Enum):
    """Enum for time formatting sizes."""
...
def format_timedelta(
    td: timedelta, size: TimeSize = TimeSize.SMALL_SPACES, max_terms: int = 99
) -> str:
...
def format_seconds(
    ...
    *,
    ...
    if days:
        word = (
            _time_str["day"][size.value] if days == 1 else _time_str["days"][size.value]
        )
        parts.append(f"{days}{word}")
    if hours or (include_zero and days):
        word = (
            _time_str["hour"][size.value]
            if hours == 1
            else _time_str["hours"][size.value]
        )
        parts.append(f"{hours}{word}")
    if minutes or (include_zero and any((hours, days))):
        word = (
            _time_str["minute"][size.value]
            if minutes == 1
            else _time_str["minutes"][size.value]
        )
        parts.append(f"{minutes}{word}")
    if seconds or (include_zero and any((minutes, hours, days))) or not parts:
        word = (
            _time_str["second"][size.value]
            if seconds == 1
            else _time_str["seconds"][size.value]
        )
    ...
    if size in [TimeSize.MEDIUM, TimeSize.MEDIUM_SPACES, TimeSize.SMALL_SPACES]:
        return " ".join(parts).strip()
    return "".join(parts).strip()


def seconds_to_dhms(seconds: float) -> str:
    """Format seconds into a DHMS string."""
    seconds = int(seconds)
